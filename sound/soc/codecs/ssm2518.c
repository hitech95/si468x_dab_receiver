...
#define SSM2518_NUM_SUPPLIES 2
static const char *ssm2518_supply_names[SSM2518_NUM_SUPPLIES] = {
	[SSM2518_SUPPLY_DVDD] = "DVDD",
	[SSM2518_SUPPLY_PVDD] = "PVDD",
};
...
struct ssm2518 {
	...
	struct regulator_bulk_data supplies[SSM2518_NUM_SUPPLIES];
...
static const unsigned int ssm2518_sysclks_2822400[] = {
	2822400, 5644800, 11289600, 16934400, 22579200, 33868800,
...
static const unsigned int ssm2518_rates_2822400[] = {
	11025, 22050, 44100,
};

static const struct snd_pcm_hw_constraint_list ssm2518_constraints_2822400 = {
	.list = ssm2518_rates_2822400,
	.count = ARRAY_SIZE(ssm2518_rates_2822400),
};
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (!ssm2518->sysclk) {
		ssm2518->sysclk = snd_soc_params_to_bclk(params);
		...
		regmap_update_bits(ssm2518->regmap, SSM2518_REG_POWER1,
			SSM2518_POWER1_NO_BCLK, SSM2518_POWER1_NO_BCLK);
		if ((ssm2518->sysclk < 2048000) || (6144000 < ssm2518->sysclk)) {
			/* If the system bit clock is in the range of acceptable internal
			* master clock frequencies (between 2.048 MHz and 6.144 MHz),
			* then it can serve as both master clock and bit clock */
			dev_err(component->dev, "bit clock %d is not acceptable\n", ssm2518->sysclk);
			ssm2518->sysclk = 0;
			return -EINVAL;
		}
	}
...
void ssm2518_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct ssm2518 *ssm2518 = snd_soc_component_get_drvdata(dai->component);
	...
	int source, unsigned int freq, int dir)
{
	...
	case 8820000:
	case 17640000:
		ssm2518->constraints = &ssm2518_constraints_2822400;
		break;
	case 3072000:
	case 6144000:
	case 38864000:
	...
	const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(ssm2518->supplies); i++)
		ssm2518->supplies[i].supply = ssm2518_supply_names[i];

	ret = devm_regulator_bulk_get(&i2c->dev, ARRAY_SIZE(ssm2518->supplies),
				      ssm2518->supplies);
	if (ret != 0) {
	...
	/* Determine PVDD voltage provided */
	ret = regulator_get_voltage(ssm2518->supplies[SSM2518_SUPPLY_PVDD].consumer);
	if (ret >= 5000000)
		regmap_update_bits(ssm2518->regmap, SSM2518_REG_MUTE_CTRL,
				SSM2518_MUTE_CTRL_ANA_GAIN, SSM2518_MUTE_CTRL_ANA_GAIN);
